...
export async function embedTexts(texts: string[], openaiKey: string): Promise<number[][]> {
...
export async function upsertCandidateEmbedding(args: {
  ...
  const { candidateId, workspaceId, document, keys } = args;
  ...
  try {
    const [embedding] = await embedTexts([document], keys.openai);
